...
public class PickupSampleFinishRR {
...
//                    double intercept = LinearRegression.intercept();
    ...
    public static void move(double x /* degrees to the left */, double y, DcMotor frontLeftMotor, DcMotor backLeftMotor,
        ...
        double calculatedYDistanceWithTrig = 15.25*(Math.tan(Math.toRadians(y)));
        double calculatedYDistanceWithData = y/3.125;
        double calculatedXDistanceWithTrig = 15.25*(Math.tan(Math.toRadians(x)));
        double calculatedXDistanceWithData = x/3.125;
