...
@Config
@Autonomous(name = "SampleAutoWithoutSpikes")
public final class SampleAutoWithoutSpikes extends LinearOpMode {
    // 1.455 slide
    // 1.374
...
//                        .splineToConstantHeading(new Vector2d(PLACEONE_X, PLACEONE_Y), Math.toRadians(-90.0), baseVelConstraint,baseAccelConstraint)
        ...
        for (int i = 0; i < 1; i++){
            telemetry.addLine("going to pick up sample");
            arm.setTargetPosition(scanArm);
            sleep(100);
            slide.setTargetPosition(scanSlide);
            sleep(100);
            wrist.setPosition(scanWrist);
            ...
            Actions.runBlocking(
                    drive.actionBuilder(lastPose)
                            ...
                            .build());
            lastPose = parkPose;
            boolean pickedUp = PickupSample.ReturnResult.pickedUp;
            while (!pickedUp && limelight.getLatestResult() != null) {
                PickupSample.pickupSample(imu, null, scanArm, scanSlide, scanWrist, arm, slide, wrist, claw,
                        limelight, telemetry, par0, par1, perp, frontLeftMotor, frontRightMotor, backLeftMotor,
                        backRightMotor, pickupArm, pickupSlide, pickupWrist, wristRotation);
                pickedUp = PickupSample.ReturnResult.pickedUp;
                if (pickedUp != pickedUpLast){
            ...
            wrist.setPosition(wristStraightUp);
            slide.setTargetPosition(slideHighBucketPosition);
            Pose2d deliverPose = new Pose2d (poseDeliverX,poseDeliverY, Math.toRadians(poseDeliverHeading));
            Actions.runBlocking(
                    drive.actionBuilder(lastPose)
                            .setTangent(Math.toRadians(180))
                            .splineToLinearHeading(deliverPose, Math.toRadians(180), curveVelConstraint, baseAccelConstraint)
                            .build());
            lastPose = deliverPose;
            if (logDetails) { detailsLog.log( detailsFilter + "," + "End Deliver Sample" ); }
            if (logAction) { sampleAction.logAction( drive.pose ); }
            if (logDetails) { detailsLog.log( detailsFilter + "," + "Start Slide to Bucket Position" ); }
            while(slide.getCurrentPosition() < (slideHighBucketPosition-100)){
